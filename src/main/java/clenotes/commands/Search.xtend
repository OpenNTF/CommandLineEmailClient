	...
	def static execute(Command cmd) {
		...
		if (cmd.hasOption("view")) {
			...
			if (subject != null) {
				...
				mailSearchString = String.format('@Contains(Subject;"%s") & !@Contains(From;"%s")', subject,
			...
			if (sender != null) {
				mailSearchString = String.format('@Contains(From;"%s") | @Contains(Principal;"%s")', sender, sender)

			//mailSearchString = '@Contains(From;"' + sender + '") | @Contains(Principal;"' + sender + '")'
			}
